...
const typeFor = e => (e.statusCode ? SERVER : e.where ? FIELD : UNKNOWN);
...
const unknown = errors =>
	logger.warn(
		'Unable to generate error message. Unrecognized error type. %o',
		errors
	);
...
export default function getMessages(errors) {
	return Object.entries(byType(errors)).reduce((acc, [type, errs]) => {
		return [...arr(acc), ...arr((FACTORIES[type] || unknown)(errs))];
	}, []);
}
