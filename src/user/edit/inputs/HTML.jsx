...
const isEmptyState = draftState =>
	!draftState || !draftState.getCurrentContent().getPlainText().trim();
...
export default class HTMLInput extends React.Component {
	...
	onContentChange = value => {
		...
		if (!this.hasChanged) {
			...
			const oldValue = arr(
				Parsers.HTML.fromDraftState(
					Parsers.HTML.toDraftState(propValue)
				)
			);
		...
		value.toJSON = () =>
			isEmptyState(value) ? null : Parsers.HTML.fromDraftState(value);
		onChange(value);
	};

	render() {
		const { label, value, schema: { readonly } = {} } = this.props;
