...
const COMPONENTS = [Choice, DateTime, Text, Bool, MultipleSelect];
...
export default function getWidget(schema) {
	...
	const Component =
		NAMES[name] ||
		COMPONENTS.find(cmp => cmp.handles && cmp.handles(schema));

	if (!Component) {
		logger.warn(
			`Unable to find a widget for '${name}' (name), '${type}' (type). Skipping it.`
		);
		return null;
	}
